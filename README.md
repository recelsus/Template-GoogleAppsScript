...
1. Install dependencies:
   ...
   npm install
   ```
2. Create an `.env` file at the repository root with at least `CLASP_SCRIPT_ID="your-script-id"`. Optional fields include `CLASP_ROOT_DIR` and `CLASP_PROJECT_ID`; omitting `CLASP_ROOT_DIR` still writes `dist` into the generated `.clasp.json`.
...
- `scripts/prebuild.mjs` reads `CLASP_SCRIPT_ID`, `CLASP_ROOT_DIR`, and `CLASP_PROJECT_ID` from the selected `.env` file and rewrites `.clasp.json`. It defaults `rootDir` to `dist` when the variable is absent. Use `--force` to overwrite an existing file.
